...
CREATE OR REPLACE FUNCTION public.is_user_member(room_id BIGINT, user_id UUID)
    RETURNS BOOLEAN AS $$
BEGIN
    RETURN EXISTS (
        SELECT 1
        FROM message_room_members mrm
                 JOIN profiles p ON mrm.profile_id = p.profile_id
        WHERE mrm.message_room_id = room_id
          AND p.profile_id = user_id
    );
END;
$$ LANGUAGE plpgsql;
...
CREATE POLICY "message_room_members_policy" ON "message_room_members" AS PERMISSIVE FOR SELECT TO "authenticated" USING (public.is_user_member("message_room_members"."message_room_id", auth.uid()));